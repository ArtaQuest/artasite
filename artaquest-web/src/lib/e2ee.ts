...
//   call  { v:2, t:"call",  room, act }             video-call invite / hangup — see below
...
//
// CALLS: `room` is 160 bits from crypto.getRandomValues, generated by the caller and never sent to
// the server in any form. Whoever holds the sealed message can join; nobody else can find the room.
// `act` distinguishes the invite from the hangup that closes it on the other screen.
...
export type ChatPayload =
  ...
  | { v: 2; t: "call"; room: string; act: "start" | "end"; dur?: number };

/** A conversation room name: 160 bits of entropy, never sent to the server in the clear. */
export function newCallRoom(): string {
  const b = crypto.getRandomValues(new Uint8Array(20));
  return "aq-" + [...b].map((x) => x.toString(16).padStart(2, "0")).join("");
}
