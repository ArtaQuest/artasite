 ...
 */
foreach ( [ 'Data', 'Schema', 'Cron', 'Secrets', 'Vault', 'Watchdog', 'Integrity', 'Health', 'Rest', 'Auth', 'Sessions', 'Account', 'Verify', 'Courses', 'Topics', 'Typology', 'Learn', 'Economy', 'Season', 'Social', 'Search', 'I18n', 'Funds', 'Extra', 'Offline', 'Notify', 'Meetings', 'Calendar', 'Booking', 'Cast', 'Assistant', 'Relay', 'Science', 'Library', 'Music', 'Motion', 'Narrate', 'Film', 'Illustration', 'Fearometer', 'Tickets', 'Stripe', 'YouTube', 'Console', 'Trends', 'Houses', 'Competitions', 'Translate', 'Artaai', 'Games', 'Challenges', 'Demo', 'Doi', 'Notebook', 'Chat', 'Rooms', 'Shell', 'Api', 'Passkey', 'News', 'Kaggle', 'Kernel', 'Gist', 'Credits', 'KaggleId', 'Cities', 'Books', 'Pdf' ] as $aq_cls ) {
...
add_action( 'plugins_loaded', function () {
	...
	AQ\Books::bootstrap();
	// Corrects one false sentence frozen into already-stored ArtaNews detections — an EMSC provenance
	// line promising coverage the request floor makes impossible, rendering live under "Where this
	// came from". Gated on option presence and idempotent; it matches nothing on the second run and
	// touches no measurement, timestamp or URL. See News::fix_emsc_labels.
	AQ\News::fix_emsc_labels();
...
add_action( 'template_redirect', function () {
	...
	// A member's own ArtaMeet calendar at /meet.ics?u=<uid>&k=<hmac> (text/calendar). A PATH, not a
	// REST route: calendar clients sniff the .ics extension and /wp-json/… has none. The same
