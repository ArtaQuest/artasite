...
class Mailer {
	...
	const TEMPLATES = [
		...
		'dm_received' => [
			...
			'body'     => "{{sender}} sent you a message, and it's still unread.\n\nWe can't show it here — ArtaQuest messages are end-to-end encrypted, so only your own device holds the key that opens them. Open Chat to read it.\n\nWe only email you when a message has been sitting unread for a few minutes while you're away — read it first and this never arrives — and at most once every 30 minutes per conversation. To stop these, turn off \"Email me when a message arrives\" in Chat.",
