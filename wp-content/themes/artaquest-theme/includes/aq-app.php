...
function aq_serve_llms_txt() {
	...
	$L[]  = '- Arta Coin (₳) is the currency: 1 ₳ = 1 mg of real gold held in full reserve — the same price in every country, redeemable for cash.';
	$L[]  = '';
...
function aq_app_route_seo( $slug ) {
	...
	$map = array(
		...
		'rankings'    => array(
			'Rankings',
			array(
				'Live standings of every open tournament on ' . $b . ' — each board ranks its entries by community hearts, and at the tournament\'s full-moon deadline the top entry takes the whole entry-fee pool (ties split evenly).',
			),
		),
		'challenges'  => array(
			'Challenges',
			array(
				'Member-made tournaments on ' . $b . ': anyone can found a challenge — choosing a category, a topic from the sitewide selector, a full-moon deadline and an entry fee — and open it with their own notebook. Every entrant pays the fee into the pool, so the field grows it together; at the full moon the most-hearted entry takes the whole pool (an exact tie splits it evenly). The Foundation never takes a cut — it runs on donations; submitting, checking and publishing are free.',
			),
		),
		'works'       => array(
			'The Feed',
...
function aq_faq_items() {
	return array(
		...
		array( 'What is Arta Coin (₳)?', "Arta Coin (₳) is ArtaQuest's currency. Each coin is backed one-for-one by a milligram of real gold held in full reserve, so it is worth the same in every country. You win coins by taking challenge pools, and you can buy or cash them out at the live gold rate from your Wallet." ),
		...
		array( 'What can I do with Arta Coins?', 'Found or enter challenges, donate them to the member fund (which also earns you donor points), or cash them out for fiat at the live gold rate from your Wallet. Every coin is one milligram of gold held in full reserve.' ),
		...
		array( 'What is a bursary for?', 'Challenge entries cost gold-backed coins, and we never want cost to bar anyone from competing. Through the Sponsors programme, donors earmark Arta Coins to a community, and bursaries draw on that fund to cover entry fees for members in low-income or underserved groups — so the door stays open without a blanket free tier. Applying takes about five minutes.' ),
		...
		array( "Where does ArtaQuest's funding come from?", 'Donations from independent individuals, plus the small spread on buying and selling coins. Submitting, checking and publishing are all free, and challenge pools are never touched: 100% of entry fees returns to the winning entrants. We deliberately do not accept money from religious organisations, governments, corporate sponsors, tech and AI companies, political organisations, or agenda-driven foundations. Every coin is gold-backed and fully accounted for: see the live Reserve and the open ledgers.' ),
		array( 'Does ArtaQuest have charitable status for tax receipts?', 'Not yet. We are a registered non-profit but have not yet obtained CRA charitable registration. We are working toward it and will notify all donors when official tax receipts become available.' ),
	);
}
...
add_action(
	...
	function () {
		...
		aq_ensure_app_page( 'challenges', 'Challenges' ); // /challenges — member-made tournaments: entry fees fund the pool, winner takes all at the full moon — pages/ChallengesPage.tsx
